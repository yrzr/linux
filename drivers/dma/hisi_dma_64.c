			       ...
			       struct hisi_dma_dev *d, bool on)
{
	...
	if (on) {
		...
		if (!timeout)
			printk(KERN_ERR "%s: channel%u timeout waiting for pause, timeout:%d\n",
			       __func__, phy->idx, timeout);
	}
}
	...
	while (stat) {
		...
		if (unlikely((err1 & BIT(i)) || (err2 & BIT(i)))) {
			...
			dev_warn(d->slave.dev,
				 "DMA ERR phy[%d] stats[0x%x] err1[0x%x] err2[0x%x]\n",
...
static int hisi_dma_start_txd(struct hisi_dma_chan *c)
{
	...
	if (BIT(c->phy->idx) & hisi_dma_get_chan_stat(d)) {
		dev_err(d->slave.dev,
			"%s: chan[%d] phy[%d] stat[0x%x]\n", __func__,
			c->vc.chan.chan_id, c->phy->idx,
...
hisi_of_dma_simple_xlate(struct of_phandle_args *dma_spec,
			 struct of_dma *ofdma)
{
...
static int hisi_dma_probe(struct platform_device *op)
{
	...
	d->phy = devm_kzalloc(&op->dev,
			      d->dma_channels * sizeof(struct hisi_dma_phy),
			      GFP_KERNEL);
	if (!d->phy)
		return -ENOMEM;
	...
	d->chans = devm_kzalloc(&op->dev,
				d->dma_requests * sizeof(struct hisi_dma_chan),
				GFP_KERNEL);
	if (!d->chans)
		return -ENOMEM;
...
static int __maybe_unused hisi_dma_pltfm_suspend(struct device *dev)
{
	...
	if (stat) {
		dev_warn(d->slave.dev,
			 "chan 0x%x is running fail to suspend\n", stat);

		for (i = d->dma_min_chan; i < d->dma_channels; i++)
			dev_warn(d->slave.dev,
				 "chanel:%d, CX_CONFIG: 0x%x, CX_AXI_CONF: 0x%x\n",
				 i, readl(d->base + CX_CONFIG + i * 0x40),
				 readl(d->base + AXI_CONFIG + i * 0x40));
		return -1;
	}
...
static int __maybe_unused hisi_dma_pltfm_resume(struct device *dev)
{
	struct hisi_dma_dev *d = dev_get_drvdata(dev);
	int ret = 0;
...
#ifdef CONFIG_PM
static const struct dev_pm_ops hisi_dma64_pm_ops = {
